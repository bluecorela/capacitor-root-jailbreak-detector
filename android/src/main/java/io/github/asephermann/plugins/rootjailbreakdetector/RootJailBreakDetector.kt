...
class RootJailBreakDetector {
    ...
    fun checkIsRooted(activity: Activity): Boolean {
        ...
        Log.d(
            Constants.LOG_TAG,
            "check c1 = isExistBuildTags: $c1"
        )
        Log.d(
            Constants.LOG_TAG,
            "check c2 = doesSuperuserApkExist: $c2"
        )
        Log.d(
            Constants.LOG_TAG,
            "check c3 = isExistSUPath: $c3"
        )
        Log.d(
            Constants.LOG_TAG,
            "check c4 = checkDirPermissions: $c4"
        )
        Log.d(
            Constants.LOG_TAG,
            "check c5 = checkExecutingCommands: $c5"
        )
        Log.d(
            Constants.LOG_TAG,
            "check c6 = checkInstalledPackages: $c6"
        )
        Log.d(
            Constants.LOG_TAG,
            "check c7 = checkforOverTheAirCertificates: $c7"
        )
    ...
    fun checkIsRootedWithEmulator(context: Context): Boolean {
        ...
        Log.d(
            Constants.LOG_TAG,
            "check c8 = isRunningOnEmulator: $c8"
        )
        ...
        get() {
            ...
            for (path in pathsArray) {
                ...
                if (fileExists) {
                    Log.d(
                        Constants.LOG_TAG,
                        String.format("[isExistSUPath] binary [%s] detected!", path)
                    )
                    result = true
                }
            }
     ...
     *
     * @see [TODO: check xdhfir RootUtils.java](https://github.com/xdhfir/xdd/blob/0df93556e4b8605057196ddb9a1c10fbc0f6e200/yeshttp/baselib/src/main/java/com/my/baselib/lib/utils/root/RootUtils.java)
     *
     * @see [TODO: check xdhfir ExecShell.java](https://github.com/xdhfir/xdd/blob/0df93556e4b8605057196ddb9a1c10fbc0f6e200/yeshttp/baselib/src/main/java/com/my/baselib/lib/utils/root/ExecShell.java)
     *
     * @see [adopted huohong01 RootUtils.java](https://github.com/huohong01/truck/blob/master/app/src/main/java/com/hsdi/NetMe/util/RootUtils.java)
     *
     * @see [adopted tansiufang54 RootUtils.java](https://github.com/tansiufang54/fncgss/blob/master/app/src/main/java/co/id/franknco/controller/RootUtil.java)
     */
    private fun checkExecutingCommands(): Boolean {
        ...
        Log.d(
            Constants.LOG_TAG,
            String.format(
                "[checkExecutingCommands] result [%s] => [c1:%s][c2:%s][c3:%s]",
                result,
                c1,
                c2,
                c3
    ...
    private fun checkInstalledPackages(context: Context): Boolean {
        ...
        for (packageInfo in installedPackages) {
            ...
            if (Constants.BLACKLISTED_PACKAGES.contains(packageName)) {
                Log.d(
                    Constants.LOG_TAG,
                    String.format(
                        "[checkInstalledPackages] Package [%s] found in BLACKLISTED_PACKAGES",
                        packageName
                    )
                )
                return true
            ...
            if (Constants.ROOT_ONLY_APPLICATIONS.contains(packageName)) {
                Log.d(
                    Constants.LOG_TAG,
                    String.format(
                        "[checkInstalledPackages] Package [%s] found in ROOT_ONLY_APPLICATIONS",
                        packageName
                    )
                )
                rootOnlyAppCount += 1
            ...
            if (Constants.CYDIA_SUBSTRATE_PACKAGE == packageName) {
                Log.d(
                    Constants.LOG_TAG,
                    String.format(
                        "[checkInstalledPackages] Package [%s] found in CYDIA_SUBSTRATE_PACKAGE",
                        packageName
                    )
                )
                rootOnlyAppCount += 1
        ...
        Log.d(
            Constants.LOG_TAG,
            String.format("[checkInstalledPackages] count of root-only apps: %s", rootOnlyAppCount)
        )
    ...
    private fun checkforOverTheAirCertificates(): Boolean {
        ...
        Log.d(
            Constants.LOG_TAG,
            String.format("[checkforOverTheAirCertificates] result: %s", result)
        )
        return result
    }
     ...
     *
     * @see [anti-emulator](https://github.com/strazzere/anti-emulator)
     *
     * @see [android-emulator-detector](https://github.com/framgia/android-emulator-detector)
     *
     * @see [testmandy RootUtil.java](https://github.com/testmandy/NativeAdLibrary-master/blob/68e1a972fc746a0b51395f813f5bcf32fd619376/library/src/main/java/me/dt/nativeadlibary/util/RootUtil.java.L59)
     */
    val isRunningOnEmulator: Boolean
        get() {
            Utils.deviceInfo
            val simpleCheck =
                (Build.MODEL.contains("Emulator") // ||Build.FINGERPRINT.startsWith("unknown") // Meizu Mx Pro will return unknown, so comment it!
                        || Build.MODEL.contains("Android SDK built for x86")
                        || Build.BOARD == "QC_Reference_Phone" || Build.HOST.startsWith("Build")) //MSI App Player
...
//                        || SystemProperties.getProp("ro.kernel.qemu") == "1"
    ...
    fun whatIsRunningOnEmulator(action: String?): Boolean {
        ...
        var result = false
        when (action) {
            "simpleCheckEmulator" -> result = Build.MODEL.contains("Emulator")
            "simpleCheckSDKBF86" -> result = Build.MODEL.contains("Android SDK built for x86")
            "simpleCheckQRREFPH" -> result = Build.BOARD == "QC_Reference_Phone"
            "simpleCheckBuild" -> result = Build.HOST.startsWith("Build")
            "checkGenymotion" -> result = Build.MANUFACTURER.contains("Genymotion")
            "checkGeneric" -> result =
                Build.FINGERPRINT.startsWith("generic") || Build.BRAND.startsWith("generic") && Build.DEVICE.startsWith(
                    "generic"
                )
            "checkGoogleSDK" -> result =
                Build.MODEL.contains("google_sdk") || "google_sdk" == Build.PRODUCT
        }
        return result
    }
    ...
    fun whatIsRooted(action: String?, context: Context): Boolean {
        var restest = false
        when (action) {
            "isExistBuildTags" -> restest = isExistBuildTags
            "doesSuperuserApkExist" -> restest = doesSuperuserApkExist()
            "isExistSUPath" -> restest = isExistSUPath
            "checkDirPermissions" -> restest = checkDirPermissions()
            "checkExecutingCommands" -> restest = checkExecutingCommands()
            "checkInstalledPackages" -> restest = checkInstalledPackages(context)
            "checkforOverTheAirCertificates" -> restest = checkforOverTheAirCertificates()
            "isRunningOnEmulator" -> restest = isRunningOnEmulator
            "simpleCheckEmulator" -> restest = whatIsRunningOnEmulator(action)
            "simpleCheckSDKBF86" -> restest = whatIsRunningOnEmulator(action)
            "simpleCheckQRREFPH" -> restest = whatIsRunningOnEmulator(action)
            "simpleCheckBuild" -> restest = whatIsRunningOnEmulator(action)
            "checkGenymotion" -> restest = whatIsRunningOnEmulator(action)
            "checkGeneric" -> restest = whatIsRunningOnEmulator(action)
            "checkGoogleSDK" -> restest = whatIsRunningOnEmulator(action)
            else -> Log.e(
                Constants.LOG_TAG,
                String.format("[WhatisRooted] action: %s", action)
            )
        }
        return restest
    }
